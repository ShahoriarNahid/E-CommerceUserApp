...
class DbHelper {
  ...
  static Future<void> updateCategoryProductCount(
    ...
    for (var cartM in cartList) {
      final catModel =
          catList.firstWhere((element) => element.name == cartM.category);
      final catDoc = _db.collection(collectionCategory).doc(catModel.id);
      wb.update(catDoc,
          {categoryProductCount: (catModel.productCount - cartM.quantity)});
    }
    return wb.commit();
  }
  ...
  static Future<void> updateProfile(String uid, Map<String, dynamic> map) {
    return _db.collection(collectionUser).doc(uid).update({'address': map});
  }
